...
def decrypt_text(encrypted_bytes: bytes, password: str):
    data = create_bitarray()
...
print("TEXT ", text.encode('utf-8'), " PASSWORD ", password)
enc_text = encrypt_text(text, password)
print("ENCRYPTED ", enc_text)
print("DECRYPTED ", decrypt_text(enc_text, password))

# data = create_bitarray()
# data.frombytes(bytes([13, 42, 81, 16, 50, 11, 0, 3]))
# key = create_bitarray()
# key.frombytes(bytes([13, 11, 16, 33, 11, 4, 12]))
